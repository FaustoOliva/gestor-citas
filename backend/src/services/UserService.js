...
export class UserService {
    ...
    putFieldUserById = async (id, field, value) => {
        ...
        try {
            ...
            console.log(response)
            if (response.rowsAffected[0] === 0) {
                return new Error(`ERROR: No se pudo actualizar el campo ${field} del usuario.`);
            }
            return text_exito;
        } catch (error) {
    ...
    createUser = async (user) => {
       ...
       try {
            ...
            const response = await pool.request()
                ...
                .query(`INSERT INTO ${userTable}(User_Lastname, User_Name, User_Email, User_Phone, User_RegisterDate, User_PasswordHash, User_IsAdmin) VALUES (@Apellido, @Nombre, @Email, @Telefono, @FechaRegistro, @PasswordHash, @EsAdmin)`);
            console.log(response)
            if (response.rowsAffected[0] === 0) {
