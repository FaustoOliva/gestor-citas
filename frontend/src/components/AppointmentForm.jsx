...
const CreateAppointmentForm = ({ onAppointmentCreated }) => {
    ...
    const currentServiceDetails = services.find(s => s.IdServicio === parseInt(selectedService));
    ...
    useEffect(() => {
        const fetchSpecies = async () => {
            ...
            try {
                ...
                setError('No se pudieron cargar las especies. Intenta de nuevo más tarde.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleServiceChange = (e) => {
        ...
        if (serviceId) {
            const service = services.find(s => s.IdServicio === parseInt(serviceId));
            if (service) {
                const groupedFields = groupFields(service.Campos);
                setDynamicFields(groupedFields);
                const initialValues = groupedFields.reduce((acc, field) => ({
                    ...acc,
        ...
        dynamicFields.length > 0 && (
            <>
                ...
                {dynamicFields.map(field => (
                    <Form.Group as={Row} className="mb-3" controlId={`form-${field.NombreCampo}`} key={field.IdCampo}>
                        <Form.Label column sm={4} className="text-sm-end">
                            {field.NombreCampo} {field.EsRequerido && <span className="text-danger">*</span>}
                        </Form.Label>
                        <Col sm={8}>
                            {field.TipoCampo === 'select' ? (
                                ...
                                <div className="d-flex gap-3 align-items-center pt-2">
                                    <Form.Check
                                        ...
                                        onChange={() => handleFieldChange({ target: { name: field.NombreCampo, value: true, type: 'radio' } })}
                                        required={field.EsRequerido}
                                    ...
                                    <Form.Check
                                        ...
                                        onChange={() => handleFieldChange({ target: { name: field.NombreCampo, value: false, type: 'radio' } })}
                                        required={field.EsRequerido}
            ...
            <Row className="justify-content-center">
                <Col md={8} lg={6}>
                    <Card className="p-4 shadow-sm">
                        <Card.Body>
                            <Card.Title className="text-center mb-4 fs-4">Crear Cita</Card.Title>
                            {loading ? (
