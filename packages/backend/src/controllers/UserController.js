...
router.delete("/:id", async (req, res) => {
  ...
  if (isNaN(userId)) {
    return res
      .status(400)
      .json({ error: "ERROR: Se debe proporcionar un ID válido." });
  }
  try {
    ...
    if (mensaje instanceof Error) {
      return res
        .status(500)
        .json({ error: "ERROR: No se pudo eliminar el usuario." });
    }
    return res
      .status(200)
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    if (data instanceof Error) {
      return res
        .status(500)
        .json({ error: "ERROR: No se pudo obtener el usuario." });
    }
...
router.get("", async (req, res) => {
  try {
    ...
    if (data instanceof Error) {
      return res
        .status(400)
        .json({ error: "ERROR: No se pudo obtener la lista de usuarios." });
    }
    return res.status(200).json(data);
  } catch (error) {
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    if (mensaje instanceof Error) {
      return res
        .status(500)
        .json({ error: "ERROR: No se pudo actualizar el usuario." });
    }
    return res
      .status(200)
